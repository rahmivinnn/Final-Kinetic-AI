...
import { Send, Paperclip, ArrowLeft, Phone, Video as VideoIcon, MoreVertical, Clock, X } from "lucide-react"
// Removed socket-provider import for frontend-only implementation
...
export default function ConversationPage() {
  ...
  // Mock socket functions for frontend-only implementation
  const isConnected = true
  const sendMessage = (message: any) => console.log('Mock send message:', message)
  const joinRoom = (roomId: string) => console.log('Mock join room:', roomId)
  const leaveRoom = (roomId: string) => console.log('Mock leave room:', roomId)
  const typingStatus = { isTyping: false, user: null }
  const setTyping = (typing: boolean) => console.log('Mock typing:', typing)
  ...
  // Check if video call should be shown from URL
  useEffect(() => {
    const videoCallParam = searchParams?.get('video')
    if (videoCallParam === 'true') {
    ...
    <div className="flex flex-col h-screen">
      ...
      <Dialog open={showVideoCall} onOpenChange={setShowVideoCall}>
        <DialogContent className="max-w-6xl h-[90vh] p-0 overflow-hidden">
          <DialogHeader className="absolute top-4 left-4 z-10 bg-white/80 backdrop-blur-sm px-4 py-2 rounded-lg">
            <DialogTitle className="flex items-center justify-between">
              <span>Video Call with {conversation.recipient.name}</span>
              <Button 
                variant="ghost" 
                size="icon" 
                className="h-8 w-8"
          ...
          <div className="h-full w-full">
            <VideoCall
              therapistName={conversation.recipient.name}
              therapistImage={conversation.recipient.avatar}
